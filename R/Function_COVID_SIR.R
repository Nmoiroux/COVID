...
parameters_SIR_COVID <- function(countryname, beta = 0.01270518, R0 = NULL, gamma = 1/14, ages=age_cl){
	...
	if (countryname %in% data_C_1 ){
		C <- CI_act_df <- read_excel("data/MUestimates_all_locations_1.xlsx", sheet = countryname) %>% as.matrix()
	} else if (countryname %in% data_C_2){
		C <- CI_act_df <- read_excel("data/MUestimates_all_locations_2.xlsx", sheet = countryname, col_names = FALSE) %>% as.matrix()
	}
...
R0x <- function(x){
	...
	R0 <- tryCatch(estimate.R(epid = x, GT = GT, begin = as.integer("1"), end = as.integer(length(x)), methods=c("ML")), error = function(e) NULL)
	if(is.null(R0)){
		return(NA) } 
	else {
